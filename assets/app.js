...
async function addStudent() {
  ...
  // نقرأ كل المجموعات ثم نحدد مجموعات الحل محليًا؛
  // هذا يدعم البيانات القديمة التي قد يكون اسمها "مجموعة الحل"
  // بينما group_type لم يتم تحديثه بعد.
  const {
    data: allGroups,
    error: solutionError
  } =
  ...
  const solGroups =
    (allGroups || []).filter(
      group =>
        ...
        String(group.name || "").includes("مجموعة الحل")
    );
...
async function createStudent(e) {
  ...
  const isSolutionGroup =
    selectedSolutionGroup.group_type === "solution" ||
    String(selectedSolutionGroup.name || "").includes("مجموعة الحل");

  if (!isSolutionGroup) {
    alert("المجموعة المختارة ليست حصة حل.");
    solutionSelect.focus();
    return;
  }

  if (
    selectedSolutionGroup.grade &&
    grade &&
    selectedSolutionGroup.grade !== grade
  ) {
    alert("اختر حصة الحل الخاصة بنفس الصف.");
    solutionSelect.focus();
    return;
  }
